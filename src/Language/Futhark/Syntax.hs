...
data AppExpBase f vn
  = -- | Function application.  Parts of the compiler expects that the
    ...
    Apply
      (ExpBase f vn)
      (NE.NonEmpty (f (Diet, Maybe VName, AutoMap), ExpBase f vn))
      SrcLoc
  | Range
      (ExpBase f vn)
      (Maybe (ExpBase f vn))
...
mkApply :: ExpBase Info vn -> [(Diet, Maybe VName, AutoMap, ExpBase Info vn)] -> AppRes -> ExpBase Info vn
